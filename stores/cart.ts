...
export const useCartStore = defineStore('cart', () => {
  ...
  async function addToCart(productId: string, quantity: number) {
    ...
    const data = await res.json()
    if (!data.success)
      throw new Error(data.statusMessage || 'Lỗi thêm giỏ hàng')
    await fetchCart()
  }
  ...
  async function updateQuantity(productId: string, quantity: number) {
    ...
    const data = await res.json()
    if (!data.success)
      throw new Error(data.statusMessage || 'Lỗi cập nhật giỏ hàng')
    await fetchCart()
  }
  ...
  async function removeFromCart(productId: string) {
    ...
    const data = await res.json()
    if (!data.success)
      throw new Error(data.statusMessage || 'Lỗi xóa sản phẩm')
    await fetchCart()
  }
