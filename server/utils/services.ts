...
import { Product, ProductCategory, Cart, Order, ForgotPassword, Account, User, AuditLog } from "./models.ts";
import { escapeRegex, hashPassword, comparePassword } from "./helpers.ts";
...
export const ProductService = {
  ...
  async getProductsClient(query: any) {
    ...
    else if (query.sort === "title_asc") sort = { title: "asc" };
    else if (query.sort === "title_desc") sort = { title: "desc" };
    ...
    const productsWithNewPrice = rawProducts.map((p: any) => {
      ...
      product.priceNew = parseFloat((product.price * (1 - product.discountPercentage / 100)).toFixed(2));
      return product;
    });
  ...
  async restoreProduct(id: string) {
    ...
    if (!product) throw new Error("Sản phẩm không tồn tại trong thùng rác");
  ...
  async processCheckout(cartId: string, userInfo: any, userId: string | null = null) {
    ...
    try {
      ...
      for (const item of cart.products) {
        ...
        if (!product || product.status !== "active" || product.deleted) {
          throw new Error(`Sản phẩm ${product ? product.title : "không xác định"} đã ngừng bán`);
        }
        ...
        // Perform Atomic inventory update within session
      ...
      // Empty Cart
      cart.products = [];
      await cart.save({ session: mongooseSession });
      ...
      // Check if error is related to MongoDB Transaction capability (standalone instances)
      ...
      if (isTransactionError) {
        console.warn("[CheckoutService] Transactions are not supported by the database deployment. Falling back to non-transactional atomic checkout.");
