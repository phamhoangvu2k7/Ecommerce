  ...
  <div class="admin-products-page">
    <div class="page-header mb-8">
      <div class="title-block">
        <h1 class="h1-title">Quản lý sản phẩm</h1>
        <p class="text-muted">Danh sách sản phẩm kinh doanh trên toàn hệ thống.</p>
      </div>
      ...
      <table class="premium-table">
        ...
        <tbody>
          <tr v-if="loading && products.length === 0">
            <td colspan="8" class="text-center py-6 text-muted">Đang tải danh sách sản phẩm...</td>
          </tr>
          <tr v-else-if="products.length === 0">
            <td colspan="8" class="text-center py-6 text-muted">Không tìm thấy sản phẩm nào.</td>
          </tr>
          <tr v-for="product in products" :key="product._id" class="table-row">
            ...
            <td class="font-semibold">{{ formatPrice(product.price) }}</td>
            <td class="text-center"><span class="discount-percent">-{{ product.discountPercentage }}%</span></td>
            ...
            <td class="text-center">
              <div class="table-actions">
                <button @click="openEditModal(product._id)" class="btn btn-secondary btn-action" title="Sửa">✏️</button>
                <button @click="handleDeleteProduct(product._id)" class="btn btn-danger btn-action" title="Xóa">🗑️</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="totalPages > 1" class="pagination-container mt-6">
      <button :disabled="currentPage === 1" @click="changePage(currentPage - 1)" class="btn btn-secondary btn-pag">
        Trước
      </button>
      <span class="pag-info">Trang {{ currentPage }} / {{ totalPages }}</span>
      <button :disabled="currentPage === totalPages" @click="changePage(currentPage + 1)" class="btn btn-secondary btn-pag">
        Sau
      </button>
    </div>
    ...
    <div v-if="showModal" class="modal-backdrop">
      <div class="modal-content premium-card glass-panel fade-in-item">
        <div class="modal-header">
          <h3 class="modal-title">{{ isEditing ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm mới' }}</h3>
          <button @click="showModal = false" class="btn btn-secondary btn-close-modal">✖</button>
        </div>
        ...
        <form @submit.prevent="handleSaveProduct" class="modal-form">
            ...
            <div class="input-group">
              ...
              <div class="upload-input-wrapper">
                ...
                <label class="btn btn-secondary btn-upload">
                  📸 Tải ảnh
                  <input type="file" @change="handleFileUpload" accept="image/*" class="file-hidden" />
                </label>
              </div>
          ...
          <div class="input-group">
            ...
            <textarea v-model="formDescription" rows="4" class="premium-input text-area-input" placeholder="Nhập mô tả sản phẩm"></textarea>
          </div>
